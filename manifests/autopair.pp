...
# @param renewal_options Extra options to set in the cert configuration files.
define serts::autopair (
  ...
  inifile::create_ini_settings($renewal_options, {
      path => "${letsencrypt::config_dir}/renewal/${fqdn}.conf"
    } + $serts::default_renewal_settings
  )
}
